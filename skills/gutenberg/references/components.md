...
```javascript
import { BlockControls, AlignmentToolbar, BlockAlignmentToolbar } from '@wordpress/block-editor';
...
<BlockControls>
	...
	/>
	<ToolbarGroup>
		<ToolbarButton icon="edit" label="Edit" onClick={() => setIsEditing(true)} />
	</ToolbarGroup>
</BlockControls>;
```
...
<Placeholder icon="admin-post" label="Block Title" instructions="Configure block settings">
...
<Button onClick={() => setIsVisible(!isVisible)}>
	Toggle Popover
	{isVisible && (
		<Popover position="bottom center" onClose={() => setIsVisible(false)}>
			Popover content
		</Popover>
	)}
</Button>;

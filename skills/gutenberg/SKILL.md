...
export default function Edit({ attributes, setAttributes }) {
	const { content, alignment, backgroundColor, showTitle, columns, mediaId, mediaUrl } = attributes;
				...
				<MediaUploadCheck>
					{!mediaUrl ? (
						<MediaUpload
							...
							value={mediaId}
							render={({ open }) => (
								<Placeholder icon="format-image" label={__('Image', 'my-plugin')}>
...
function my_plugin_register_patterns()
{
	...
	register_block_pattern_category('my-plugin', ['label' => __('My Plugin Patterns', 'my-plugin')]);
}
```
...
registerBlockVariation('core/group', {
	...
	isActive: (blockAttributes) => blockAttributes.className?.includes('is-style-card'),
});
```
...
addFilter('blocks.registerBlockType', 'my-plugin/modify-block', (settings, name) => {
...
addFilter('blocks.registerBlockType', 'my-plugin/add-attributes', (settings, name) => {
	if (name === 'core/image') {
		settings.attributes = {
			...settings.attributes,
			customCaption: {
	...
	return (props) => {
		...
		return (
			<>
				<BlockEdit {...props} />
				<InspectorControls>
					<PanelBody title="Custom Settings">{/* Custom controls */}</PanelBody>
				</InspectorControls>
			</>
		);
	};

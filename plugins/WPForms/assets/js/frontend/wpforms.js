	...
	(function (document, window, $) {
		...
		const app = {
			...
			loadHoneypot() {
				$('.wpforms-form').each(function () {
					...
					const insertBeforeId = fieldIds[Math.floor(Math.random() * fieldIds.length)],
						...
						inlineStyles =
							'position: absolute !important; overflow: hidden !important; display: inline !important; height: 1px !important; width: 1px !important; z-index: -1000 !important; padding: 0 !important;',
					...
					const $fieldContainer = $(
						`#wpforms-${formId}-field_${wpforms_settings.hn_data[formId]}-container`,
						$form
					);
			...
			loadValidation() {
				...
				$.validator.messages.min = getSetting(
					'val_min',
					'Please enter a value greater than or equal to {0}'
				).replace('{value}', '{0}');
				$.validator.messages.max = getSetting(
					'val_max',
				...
				$.validator.addMethod(
					...
					function (value, element) {
						...
						const isRequired =
							element.hasAttribute('required') || $field.hasClass('wpforms-field-required');
						...
						const hasFile =
							...
							$field.find('.wpforms-camera-selected-file.wpforms-camera-selected-file-active')
								.length > 0;
				...
				$.validator.addMethod(
					...
					function (value, element) {
						...
						return (
							this.optional(element) || /^((0?[1-9]|1[012])(:[0-5]\d){1,2}(\ ?[AP]M))$/i.test(value)
				...
				$.validator.addMethod(
					...
					function (value, element) {
						...
						return (
							...
							/^(([0-1]?[0-9])|([2][0-3])):([0-5]?[0-9])(\ ?[AP]M)?$/i.test(value)
						); // eslint-disable-line no-useless-escape
					},
				...
				$.validator.addMethod(
					...
					function (value, element) {
						...
						if (app.compareTimesGreaterThan(maxTime, minTime)) {
							return (
								app.compareTimesGreaterThan(value, minTime) &&
								app.compareTimesGreaterThan(maxTime, value)
							);
						}

						return (
							(app.compareTimesGreaterThan(value, minTime) &&
								app.compareTimesGreaterThan(value, maxTime)) ||
							(app.compareTimesGreaterThan(minTime, value) &&
								app.compareTimesGreaterThan(maxTime, value))
						);
					},
				...
				$.validator.addMethod(
					...
					function (value, element, param) {
						 ...
						 */
						return (
							(value === '' && !$el.hasClass('wpforms-field-required')) ||
							Number(app.amountSanitize(app.amountFormat(param))) <=
								Number(app.amountSanitize(value))
						);
					},
				...
				$.validator.addMethod(
					...
					function (value, element) {
						...
						return (
							(value === '' && !$el.hasClass('wpforms-field-required')) ||
							strength >= Number($el.data('password-strength-level'))
						);
					},
				...
				$('.wpforms-validate').each(function () {
					...
					if (
						...
						properties = {
							...
							ignore:
								':hidden:not(textarea.wp-editor-area):not(.wpforms-field-camera:not(.wpforms-conditional-hide) input), .wpforms-conditional-hide textarea.wp-editor-area',
							ignoreTitle: true,
							errorPlacement(error, element) {
								...
								if (app.isLikertScaleField(element)) {
									...
									error.insertAfter(
										element.parent().find('p.wpforms-file-upload-capture-camera-classic')
									);
								} else {
							...
							unhighlight(element, errorClass, validClass) {
								...
								if (
									!$field.find(':input.wpforms-error,[data-dz-errormessage]:not(:empty)').length
								) {
							...
							submitHandler(form) {
								...
								const disableSubmitButton = function ($form) {
									...
									WPFormsUtils.triggerEvent($form, 'wpformsFormSubmitButtonDisable', [
										$form,
										$submit,
									]);
								};
								 ...
								 */
								const submitHandlerRoutine = function () {
									// eslint-disable-line complexity
									const $form = $(form),
							...
							onkeyup: WPFormsUtils.debounce(function (element, event) {
			...
			loadDatePicker($context) {
				...
				$context.find('.wpforms-datepicker-wrap').each(function () {
					...
					if (
						...
						window['wpforms_' + formID + '_' + fieldID].hasOwnProperty('datepicker')
					) {
			...
			loadTimePicker($context) {
				...
				$context.find('.wpforms-timepicker').each(function () {
					// eslint-disable-line complexity
					const element = $(this),
						form = element.closest('.wpforms-form'),
						formID = form.data('formid'),
				...
				$(document).on('blur', '.wpforms-field-email input', function () {
					...
					$input.mailcheck({
						suggested($el, suggestion) {
							...
							const address = decodeURI(suggestion.address)
									.replaceAll(/[<>'"()/\\|:;=@%&\s]/gi, '')
									.substr(0, 64),
								domain = decodeURI(suggestion.domain).replaceAll(/[<>'"()/\\|:;=@%&+_\s]/gi, '');
			...
			loadChoicesJS($context) {
				...
				$context
					...
					.each(function (idx, el) {
						 ...
						 */
						const event = WPFormsUtils.triggerEvent($context, 'wpformsBeforeLoadElementChoices', [
							el,
						]);
			...
			bindUIActions() {
				...
				$document.on(
					...
					function (event) {
						...
						hintEl.html(
							hintEl.data('hint').replaceAll('{value}', '<b>' + event.target.value + '</b>')
						);
					}
				);
			...
			skipEmptyPages(event, nextPage, $form, action) {
				...
				wpforms.navigateToPage(
					$previousPage.find('.wpforms-page-next'),
					'next',
					prevPage,
					$form,
					$previousPage
			...
			isEmptyPage($form, page) {
				...
				const $fieldsOnPage = $currentPage.find(
			...
			navigateToPage($this, action, page, $form, $page) {
				...
				const event = WPFormsUtils.triggerEvent($this, 'wpformsBeforePageChange', [
					nextPage,
					$form,
					action,
				]);
			...
			updateOrderSummaryItem($paymentField, $summary) {
				...
				if (type === 'checkbox' || type === 'radio' || type === 'select-one') {
					...
					$item
						...
						.text(app.amountFormatSymbol(app.amountSanitize(amount)));
			...
			updateOrderSummaryItemQuantity($input) {
				...
				$form.find('.wpforms-order-summary-preview').each(function () {
					...
					$item
						.find('.wpforms-order-summary-item-price')
						.text(app.amountFormatSymbol(amount * quantity));
			...
			amountTotalCalc($form) {
				 ...
				 */
				const event = WPFormsUtils.triggerEvent($document, 'wpformsAmountTotalCalculate', [
					$form,
					total,
				]);
			...
			getPaymentFieldAmount($field) {
				...
				if (
					...
					$field.find('option:selected').data('amount')
				) {
			...
			formSubmitNormal($form) {
				...
				$form.append(
					'<input type="hidden" name="start_timestamp" value="' +
						app.getStartTimestampData($form) +
						'">'
				);
				$form.append(
					'<input type="hidden" name="end_timestamp" value="' + app.getTimestampSec() + '">'
				);
			 ...
			 */
			displayFormAjaxErrors($form, errors) {
				// eslint-disable-line complexity
			 ...
			 */
			displayFormAjaxGeneralErrors($form, errors) {
				// eslint-disable-line complexity
				if (!$form || !$form.length) {
					return;
				}
				...
				if ('string' === typeof errors) {
					const roleAttr = app.isModernMarkupEnabled() ? ' role="alert"' : '',
						errPrefix = app.isModernMarkupEnabled()
			...
			setCurrentPage($form, $json) {
				...
				const $currentPage =
					$errorPages.length > 0 ? $errorPages[0] : $form.find('.wpforms-page-1');
				...
				if (
					currentPage === 1 ||
					($json.errors !== undefined && $json.errors.general.footer !== undefined)
				) {
			...
			animateScrollTop(position, duration, complete) {
				...
				return $('html, body')
					.animate({ scrollTop: parseInt(position, 10) }, { duration, complete })
					.promise();
			},
			...
			initTokenUpdater() {
				...
				); // Bind `this` to maintain context inside the function
			},

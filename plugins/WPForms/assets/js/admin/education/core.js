		...
		const app = {
			...
			openModalButtonClick() {
				$(document)
					.on(
						'click',
						'.education-modal:not(.wpforms-add-fields-button)',
						app.openModalButtonHandler
					)
			 ...
			 */
			proFieldDelete() {
				$('#wpforms-builder').on('wpformsFieldDelete', function () {
			 ...
			 */
			setDykColspan() {
				$('#adv-settings').on('change', 'input.hide-column-tog', function () {
			...
			gotoAdvancedTabClick() {
				$(document).on('click', '.wpforms-educational-alert.wpforms-calculations a', function (e) {
					const $a = $(this);
			...
			activateModal($button) {
				...
				$.alert({
					...
					content: message
						? message
						: wpforms_education.activate_prompt.replace(/%name%/g, feature),
					icon: 'fa fa-info-circle',
					type: 'blue',
					buttons: {
						confirm: {
							...
							action() {
								this.$$confirm.prop('disabled', true).html(spinner + wpforms_education.activating);
				...
				$.post(
					...
					function (res) {
						...
						if (res.success) {
							...
							app.saveModal(
								pluginType === 'plugin'
									? wpforms_education.plugin_activated
									: wpforms_education.addon_activated
							);
						} else {
							$.alert({
								title: false,
					...
					buttons: {
						confirm: {
							...
							action() {
								...
								this.$$confirm.prop('disabled', true).html(spinner + wpforms_education.saving);
				...
				$.alert({
					...
					buttons: {
						confirm: {
							...
							action() {
								this.$$confirm.prop('disabled', true).html(spinner + wpforms_education.installing);
					...
					buttons: {
						confirm: {
							...
							action() {
								...
								this.$$confirm.prop('disabled', true).html(spinner + this.$$confirm.text());

...
const wpf = {
	...
	getChoicesOrder(id) {
		...
		jQuery('#wpforms-field-option-' + id)
			.find('.choices-list li')
			.each(function () {
	...
	getFields(
		...
		useCache = undefined,
		isAllowedRepeaterFields = undefined,
	...
	addAdditionalFields(fields) {
		for (const key in fields) {
			...
			if (fields[key]?.type === 'address') {
				...
				const addressFields = Object.keys(fields[key]).filter((fieldKey) =>
					fieldKey.includes('_placeholder')
				);
		...
		if (wpf.getQueryString(name)) {
			...
			history.replaceState &&
				history.replaceState(
					null,
					'',
					location.pathname + location.search.replace(re, '').replace(/^&/, '?') + location.hash
				);
		}
	},
	...
	amountSanitize(amount) {
		...
		amount = String(amount)
			.replace(wpforms_builder.currency_symbol, '')
			.replace(/[^0-9.,]/g, '');

		if (wpforms_builder.currency_decimal === ',') {
			if (
				wpforms_builder.currency_thousands === '.' &&
				amount.indexOf(wpforms_builder.currency_thousands) !== -1
			) {
	...
	amountFormat(amount) {
		...
		if (
			...
			amount.indexOf(wpforms_builder.currency_decimal) !== -1
		) {
	...
	isDebug() {
		return (
			(window.location.hash && '#wpformsdebug' === window.location.hash) ||
			window.wpforms_builder?.debug
		);
	},
	...
	initTooltips($scope = null) {
		...
		const isRTL = jQuery('body').hasClass('rtl'),
			...
			$tooltips = $scope
				? jQuery($scope).find('.wpforms-help-tooltip')
				: jQuery('.wpforms-help-tooltip');
	...
	restoreTooltips($scope) {
		$scope =
			typeof $scope !== 'undefined' && $scope && $scope.length > 0
				? $scope.find('.wpforms-help-tooltip')
				: jQuery('.wpforms-help-tooltip');
	...
	decodeAllowedHTMLEntities(string) {
		 ...
		 */
		const allowedEntities = wp.hooks.applyFilters('wpforms.allowedHTMLEntities', {
			'&amp;': '&',
			'&nbsp;': ' ',
		});
	...
	triggerEvent($element, eventName) {
		// eslint-disable-next-line no-console
		console.warn(
			'WARNING! Function "wpf.triggerEvent( $element, eventName )" has been deprecated, please use the new "WPFormsUtils.triggerEvent( $element, eventName, args )" function instead!'
		);
	...
	wpautop(pee, br = true) {
		...
		if (pee.indexOf('<pre') > -1) {
			...
			peeParts.forEach(function (peePart, index) {
		...
		if (pee.indexOf('<option') > -1) {
			...
			pee = pee.replace(/(?=(\s*))\2<option'/gim, '<option');
		...
		if (pee.indexOf('</object>') > -1) {
			...
			pee = pee.replace(/(?=(\s*))\2<\/object>/gim, '</object>');
			pee = pee.replace(/(?=(\s*))\2(<\/?(?:param|embed)[^>]*>)((?=(\s*))\2)/gim, '$1');
		}
		...
		if (pee.indexOf('<source') > -1 || pee.indexOf('<track') > -1) {
			...
			pee = pee.replace(/(?=(\s*))\2([<\[]\/(?:audio|video)[>\]])/gim, '$1');
			pee = pee.replace(/(?=(\s*))\2(<(?:source|track)[^>]*>)(?=(\s*))\2/gim, '$1');
		}
		...
		pee = pee.replace(/<p>\s*<\/p>/gim, ''); // Under certain strange conditions, it could create a P of entire whitespace.
		...
		if (br) {
			pee = pee.replace(/<(script|style)(?:.|\n)*?<\/\\1>/gim, _autopNewlinePreservationHelper); // /s modifier from php PCRE regexp replaced with (?:.|\n).
		...
		if (Object.keys(preTags).length) {
			pee = pee.replace(new RegExp(Object.keys(preTags).join('|'), 'gi'), function (matched) {
	...
	initMediaLibrary(args) {
		...
		mediaFrame
			.on('uploader:ready', function () {
				...
				jQuery('.wpforms-media-frame .moxie-shim-html5 input[type="file"]').attr('accept', accept);
			})
	...
	copyValueToClipboard(event, $copyButton, targetElement) {
		...
		if (navigator.clipboard) {
			navigator.clipboard.writeText(targetElement.val()).then(function () {
	...
	_getCurrentFormState() {
		...
		const currentState = Object.fromEntries(
			new FormData(document.getElementById('wpforms-builder-form')).entries()
		);

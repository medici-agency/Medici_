...
WPFormsChallenge.embed =
	window.WPFormsChallenge.embed ||
	(function (document, window, $) {
		...
		var app = {
			...
			load() {
				...
				if (WPFormsChallenge.core.isGutenberg()) {
					...
					app.onElementReady('.block-editor .edit-post-header', function () {
						WPFormsChallenge.core.initTooltips(5, '.block-editor .edit-post-header', {
							side: 'bottom',
						});
			 ...
			 */
			events: function () {
				$('.wpforms-challenge-step5-done').on('click', app.lastStep);

				$('.wpforms-challenge-popup-close, .wpforms-challenge-end').on(
					'click',
					app.completeChallenge
				);

				$('#wpforms-challenge-contact-form .wpforms-challenge-popup-contact-btn').on(
					'click',
			...
			showPopup: function () {
				...
				if (0 < secondsLeft) {
					...
					$('#wpforms-challenge-congrats-minutes').text(
						WPFormsChallenge.core.timer.getMinutesFormatted(secondsSpent)
					);
					$('#wpforms-challenge-congrats-seconds').text(
						WPFormsChallenge.core.timer.getSecondsFormatted(secondsSpent)
					);
			...
			completeChallenge: function () {
				...
				WPFormsChallenge.admin
					.saveChallengeOption(optionData)

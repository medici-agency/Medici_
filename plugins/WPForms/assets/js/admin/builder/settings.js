...
WPForms.Admin.Builder.Settings =
	WPForms.Admin.Builder.Settings ||
		...
		var app = {
			...
			events() {
				el.$panel
					.on(
						'keydown',
						'#wpforms-panel-field-settings-form_tags-wrap input',
						app.addCustomTagInput
					)
					.on(
						'removeItem',
						'#wpforms-panel-field-settings-form_tags-wrap select',
						app.editTagsRemoveItem
					)
				...
				el.$selectTags.on('change', app.changeTags);
			...
			disableEntries() {
				...
				const isStoreSpamEntriesChecked = $(
					'#wpforms-panel-field-settings-store_spam_entries'
				).prop('checked');

				app.toggleFilteringMessages(!isChecked && isStoreSpamEntriesChecked);
				// Toggle the store spam entries toggle.
				$('#wpforms-panel-field-settings-store_spam_entries-wrap').toggleClass(
					'wpforms-hidden',
					$this.prop('checked')
				);
			...
			toggleFilteringMessages($hide) {
				...
				$('#wpforms-panel-field-anti_spam-country_filter-message-wrap').toggleClass(
					'wpforms-hidden',
					$hide
				);

				// Toggle Keyywords Filter Message.
				$('#wpforms-panel-field-anti_spam-keyword_filter-message-wrap').toggleClass(
					'wpforms-hidden',
					$hide
				);
			},
			...
			initTags($el = null) {
				...
				if (!$el.length || typeof window.Choices !== 'function') {
					return;
				}
				...
				vars.tagsChoicesObj
					.clearStore()
					.setChoices(wpforms_builder_settings.all_tags_choices, 'value', 'label', true)
			...
			initTagsHiddenInput() {
				...
				el.$selectTags.closest('.wpforms-panel-field').append(el.$selectTagsHiddenInput);
			...
			changeTags(event) {
				...
				el.$selectTagsHiddenInput.val(JSON.stringify(tags));
			},
			...
			getDisabledEntryMessage(entryRequirement) {
				...
				const dependenciesHTML = Object.values(dependencies)
			 ...
			 */
			getEntryRequirement() {
				return wp.hooks.applyFilters('wpforms.Builder.entryRequirement', {
					required: false,
					dependencies: {},

 ...
 * @since 1.9.8.3
 */
WPForms.Admin.Builder.MultipleChoices =
	WPForms.Admin.Builder.MultipleChoices ||
		...
		const app = {
			...
			bindEvents() {
				...
				app.$builder.on(
					...
					app.toggleOtherOption
				);

				// Real-time update Other input placeholder in preview.
				app.$builder.on(
					'input',
					'.wpforms-field-option-row-other_placeholder input',
					app.updateOtherOptionPlaceholder
				);

				// Real-time update Other input size in preview.
				app.$builder.on(
					'change',
					'.wpforms-field-option-row-other_size select',
					app.updateOptionInputFieldSize
				);
				...
				app.$builder.on(
					'input',
					'.wpforms-field-option-row-choices li.wpforms-choice-other-option input.value',
					app.updateOptionInputPreview
				);

				// When Dynamic Choices is toggled ON, force-disable Other option.
				app.$builder.on(
					'change',
					'.wpforms-field-option-row-dynamic_choices select',
					app.toggleDynamicChoices
				);
			...
			updateOptionInputPreview() {
				...
				const showValuesOn = $options
					.find('.wpforms-field-option-row-show_values input')
					.is(':checked');
				...
				const placeholder =
					$options.find('.wpforms-field-option-row-other_placeholder input').val() || '';
			...
			toggleOtherOption(e) {
				...
				if (checked) {
					const sizeVal =
						$fieldOptions.find('.wpforms-field-option-row-other_size select').val() || 'medium';
			...
			createOtherChoice($choicesList, fieldID, key) {
				...
				$clone
					.find('input.label')
					.val(otherLabel)
					.attr('name', `fields[${fieldID}][choices][${key}][label]`);
				$clone
					.find('input.value')
					.val('')
					.attr('name', `fields[${fieldID}][choices][${key}][value]`);
				$clone
					.find('.wpforms-image-upload input.source')
					.val('')
					.attr('name', `fields[${fieldID}][choices][${key}][image]`);
				$clone
					.find('.wpforms-icon-select input.source-icon')
					.val(wpforms_builder.icon_choices.default_icon)
					.attr('name', `fields[${fieldID}][choices][${key}][icon]`);
				$clone
					.find('.wpforms-icon-select input.source-icon-style')
					.val(wpforms_builder.icon_choices.default_icon_style)
					.attr('name', `fields[${fieldID}][choices][${key}][icon_style]`);
				$clone
				...
				$clone
					.find('input.default')
					.attr('name', `fields[${fieldID}][choices][${key}][default]`)
					.prop('checked', false);
			...
			updatePreviewState(fieldId) {
				...
				const $otherRadio = $options.find(
					'.choices-list li.wpforms-choice-other-option input[type="radio"]'
				);
				...
				const val =
					$options.find('.choices-list li.wpforms-choice-other-option input.value').val() || '';

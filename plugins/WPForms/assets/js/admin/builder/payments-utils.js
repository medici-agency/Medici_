...
const WPFormsBuilderPaymentsUtils =
	window.WPFormsBuilderPaymentsUtils ||
		...
		const app = {
			...
			toggleContent() {
				...
				if (
					$paymentSettings
						.find(
							'.wpforms-panel-content-section-payment-toggle-one-time .wpforms-toggle-control > input'
						)
						.is(':checked') &&
					$paymentSettings
						.find(
							'.wpforms-panel-content-section-payment-toggle-recurring .wpforms-toggle-control > input'
						)
						.is(':checked')
				) {
					...
					$.alert({
						...
						content: $input.attr('name').includes('enable_recurring')
							? wpforms_builder_payments_utils.payments_disabled_recurring
							: wpforms_builder_payments_utils.payments_disabled_one_time,
				...
				const $wrapper = $input.closest('.wpforms-panel-content-section-payment'),
					isChecked =
						$input.prop('checked') &&
						!$('#wpforms-panel-field-settings-disable_entries').prop('checked');
			...
			togglePlan() {
				...
				$plan
					...
					.toggle($icon.hasClass('fa-chevron-circle-down'));
			},
			...
			deletePlan() {
				...
				$(this)
					.closest('.wpforms-panel-content-section-payment')
					.find('.wpforms-panel-content-section-payment-button-add-plan')
					.trigger('click');
			},

...
WPForms.Admin.Builder.DragFields =
	WPForms.Admin.Builder.DragFields ||
	(function (document, window, $) {
		...
		const app = {
			...
			setup() {
				...
				el = {
					...
					$addFieldsButtons: $('.wpforms-add-fields-button')
						.not('.not-draggable')
						.not('.warning-modal')
						.not('.education-modal'),
				};
			 ...
			 */
			async layoutPresetChanged(event, fieldOptions) {
				...
				$sortable.sortable({
					...
					stop(e, ui) {
						...
						if (!isNewField && $field.closest('.wpforms-layout-column').is($sortable)) {
							fieldLayout.positionFieldInColumn(fieldId, $field.index() - 1, $sortable);
						}
						...
						if (!isNewField) {
							$field.removeClass('wpforms-field-dragging').removeClass('wpforms-field-drag-over');
						}
					...
					over(e, ui) {
						...
						$field.addClass('wpforms-field-dragging' + targetClass);
						...
						$placeholder.removeClass('wpforms-field-drag-not-allowed').css({
						...
						$field.addClass('wpforms-field-drag-over').removeClass('wpforms-field-drag-out');
					},
					out(e, ui) {
						const $field = ui.item.first(),
						...
						$field.addClass('wpforms-field-drag-out').removeClass('wpforms-field-drag-over');
					},
					receive(e, ui) {
						// eslint-disable-line complexity
						...
						// Drop to this place is not allowed.
						if (isColumn && !fieldLayout.isFieldAllowedInColum(fieldType, $sortable)) {
							vars.fieldRejected = true;
						...
						// Move existing field.
						if (!isNewField) {
							fieldLayout.receiveFieldToColumn(fieldId, ui.item.index() - 1, $field.parent());
						...
						WPFormsBuilder.fieldAdd(vars.fieldType, {
			...
			initDraggableFields() {
				el.$addFieldsButtons.draggable({
					...
					start(e, ui) {
						const event = WPFormsUtils.triggerEvent(el.$builder, 'wpformsFieldAddDragStart', [
							vars.fieldType,
							ui,
						]);
					...
					stop(e, ui) {
						const event = WPFormsUtils.triggerEvent(el.$builder, 'wpformsFieldAddDragStop', [
							vars.fieldType,
							ui,
						]);
		...
		fieldLayout = {
			...
			isFieldAllowedInColum(fieldType, $targetColumn) {
				...
				const isAllowed = WPForms.Admin.Builder.FieldLayout.isFieldAllowedInColum(
					fieldType,
					$targetColumn
				);

...
const WPFormsSettingsStripe =
	window.WPFormsSettingsStripe ||
	(function (document, window, $) {
		...
		const app = {
			...
			setup() {
				...
				el.copyButton = $(
					'#wpforms-setting-row-stripe-webhooks-endpoint-set .wpforms-copy-to-clipboard'
				);
			 ...
			 */
			bindEvents() {
				el.$wrapper.on('change', '#wpforms-setting-stripe-test-mode', app.triggerModeSwitchAlert);
				el.copyButton.on('click', function (e) {
					wpf.copyValueToClipboard(e, $(this), el.webhookEndpointUrl);
				});
				el.webhookMethod.on('change', app.onMethodChange);
			},
			...
			copyWebhooksEndpoint(event) {
				...
				console.warn(
					'WARNING! Function "WPFormsSettingsStripe.copyWebhooksEndpoint()" has been deprecated! Use wpf.copyWebhooksEndpoint() instead.'
				);

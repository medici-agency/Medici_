 ...
 * @since 1.8.2
 */
var WPFormsStripeElements =
	window.WPFormsStripeElements ||
		...
		const app = {
			...
			updateFormSubmitHandler: function ($form) {
				...
				formValidator.settings.submitHandler = function () {
					let valid = $form.validate().form(),
						...
						condHidden = $stripeDiv
							.closest('.wpforms-field-stripe-credit-card')
			...
			handleCardActionCallback: function (e, json) {
				...
				if (json.success && json.data.action_required) {
					app.stripe
						.handleCardPayment(json.data.payment_intent_client_secret, {
			...
			handleCardPaymentCallback($form, result) {
				if (result.error) {
					...
					$form
						.find('.wpforms-field-stripe-credit-card-cardnumber')
						.addClass(wpforms_stripe.data.element_classes.invalid);
					app.displayStripeError($form, result.error.message);
			...
			formAjaxUnblock($form) {
				...
				console.warn(
					'WARNING! Function "WPFormsStripeElements.formAjaxUnblock()" has been deprecated, please use the new "wpforms.restoreSubmitButton()" function instead!'
				);
			...
			updateCardElementStylesModern($form) {
				...
				if (
					...
					!$.isEmptyObject(wpforms_stripe.data.element_style)
				) {
					return;
				}

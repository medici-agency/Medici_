	...
	(function (document, window, $) {
		...
		const app = {
			...
			bindUIActions() {
				...
				$('#wpforms-panel-field-stripe-recurring-email').on(
					'change',
					app.resetEmailAlertErrorClass
				);
			},
			...
			settingsDisplay() {
				...
				if (ccFieldsAdded.length) {
					...
					$content
						.find(
							'#wpforms-stripe-new-interface-alert, .wpforms-stripe-notice-info, .wpforms-panel-field, .wpforms-conditional-block-panel, h2'
						)
						.show();
				} else {
					$alert.show();
					$content
						...
						.hide();
					$content
						.find('#wpforms-panel-field-stripe-enable')
						.prop('checked', false)
						.trigger('change');
				}
			},
			...
			settingsConditions() {
				...
				$('#wpforms-panel-field-stripe-recurring-enable').conditions({
					...
					actions: {
						if: {
							element:
								'#wpforms-panel-field-stripe-recurring-period-wrap,#wpforms-panel-field-stripe-recurring-conditional_logic-wrap,#wpforms-conditional-groups-payments-stripe-recurring,#wpforms-panel-field-stripe-recurring-email-wrap,#wpforms-panel-field-stripe-recurring-name-wrap',
							action: 'show',
						},
						else: {
							element:

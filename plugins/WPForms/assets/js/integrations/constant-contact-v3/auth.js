 ...
 * @since 1.9.3
 */
const WPFormsConstantContactV3Auth =
	window.WPFormsConstantContactV3Auth ||
	(function (document, window, $) {
		...
		const app = {
			 ...
			 */
			ready: () => {
				const redirectUri = new URL(WPFormsConstantContactV3AuthVars.auth_url).searchParams.get(
					'redirect_uri'
				);
				...
				$(document)
					.on(
						'click',
						'.wpforms-constant-contact-v3-auth, .wpforms-builder-constant-contact-v3-provider-sign-up',
						app.showWindow
					)
					.on(
						'click',
						'#wpforms-settings-constant-contact-v3-migration-prompt-link',
						app.promptMigration
					);
			},
			...
			saveAccount: (code) => {
				...
				$.post(WPFormsConstantContactV3AuthVars.ajax_url, {

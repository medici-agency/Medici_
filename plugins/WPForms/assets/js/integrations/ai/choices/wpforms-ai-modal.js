	...
	(function (document, window, $) {
		...
		const app = {
			...
			initChoicesModal() {
				...
				args.contentMaxHeight = Math.min(
					app.defaultOptions.contentMaxHeight,
					app.getMaxModalHeight()
				);
			...
			resizeModalHeight(fieldId) {
				...
				const $modal = $('.jconfirm-wpforms-ai-modal').filter(function () {
					...
					return $(this)
						.attr('class')
						.match(new RegExp('jconfirm-wpforms-ai-modal-.*-' + fieldId, 'i'));
				});

				$modal.find('.jconfirm-content-pane').css({
					height: modalHeight,
			...
			refreshModalHeight(e) {
				...
				$(`.jconfirm-wpforms-ai-modal-choices-${fieldId} .jconfirm-content-pane`).css({
					height: maxHeight,

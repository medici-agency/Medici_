 ...
 * @since 1.9.5
 */
const WPFormsSettingsSquare =
	window.WPFormsSettingsSquare ||
	(function (document, window, $) {
		...
		const $el = {
			...
			productionConnectionStatusBlock: $(
				'#wpforms-setting-row-square-connection-status-production'
			),
			...
			webhookCommunicationStatusNotice: $(
				'#wpforms-setting-row-square-webhooks-communication-status'
			),
			webhookConnectBtn: $('#wpforms-setting-square-webhooks-connect'),
			webhookConnectRow: $('#wpforms-setting-row-square-webhooks-connect'),
			webhookConnectStatusRow: $(
				'#wpforms-setting-row-square-webhooks-connect-status-production, #wpforms-setting-row-square-webhooks-connect-status-sandbox'
			),
		};
		...
		const app = {
			...
			refreshTokensCallback() {
				...
				const settings = {
					...
					beforeSend() {
						$btn
							...
							.html(WPFormsAdmin.settings.iconSpinner)
							.prop('disabled', true);
					},
				};
				...
				$.ajax(settings)
					.done(function (response) {
						...
						$btn.css('width', 'auto').html(buttonLabel).prop('disabled', false);
			...
			credentialsFieldsDisplay() {
				...
				if (
					sandboxModeEnabled &&
					$el.sandboxConnectionStatusBlock.find('.wpforms-square-connected').length
				) {
					return;
				}
				...
				if (
					!sandboxModeEnabled &&
					$el.productionConnectionStatusBlock.find('.wpforms-square-connected').length
				) {
					return;
				}
			...
			modals: {
				...
				displayWebhookConfigPopup() {
					$.confirm({
						...
						content:
							wpforms_admin.square.webhook_create_description +
							'<input type="text" id="wpforms-square-personal-access-token" placeholder="' +
							wpforms_admin.square.webhook_token_placeholder +
							'" value="">' +
							'<p class="wpforms-square-webhooks-connect-error error" style="display:none;">' +
							wpforms_admin.square.token_is_required +
							'</p>',

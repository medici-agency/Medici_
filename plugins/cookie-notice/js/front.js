...
(function (window, document, undefined) {
	var cookieNotice = new (function () {
		...
		this.setStatus = function (cookieValue) {
			...
			if (cnArgs.onScroll) window.removeEventListener('scroll', this.handleScroll);
			...
			if (cnArgs.globalCookie) cookieDomain = this.getDomain(document.location.hostname);
			...
			if (document.location.protocol === 'https:') secureValue = ';secure';
			...
			var event = new CustomEvent('setCookieNotice', {
				detail: {
					value: cookieValue,
					time: date,
					expires: laterDate,
			...
			this.setBodyClass([
				'cookies-set',
				cookieValue === 'true' ? 'cookies-accepted' : 'cookies-refused',
			]);
			...
			if (
				cnArgs.redirection &&
				((cookieValue === 'true' && this.cookiesAccepted === null) ||
					(cookieValue !== this.cookiesAccepted && this.cookiesAccepted !== null))
			) {
		...
		this.getDomain = function (url) {
			...
			if (!regex.test(url)) url = 'http://' + url;
		...
		this.showCookieNotice = function () {
			...
			// trigger custom event
			var event = new CustomEvent('showCookieNotice', {
				detail: {
					data: cnArgs,
				},
			});
		...
		this.hideCookieNotice = function () {
			...
			// trigger custom event
			var event = new CustomEvent('hideCookieNotice', {
				detail: {
					data: cnArgs,
				},
			});
		...
		this.showRevokeNotice = function () {
			...
			// trigger custom event
			var event = new CustomEvent('showRevokeNotice', {
				detail: {
					data: cnArgs,
				},
			});
		...
		this.handleScroll = function () {
			...
			// accept cookie
			if (scrollTop > parseInt(cnArgs.onScrollOffset)) this.setStatus('accept');
		};
		...
		this.init = function () {
			...
			if (this.cookiesAccepted === null) {
				...
				if (cnArgs.onClick)
					window.addEventListener(
						...
						function (e) {
							...
							if (outerContainer === null) _this.setStatus('accept');
						},
						true
					);
				...
				this.setBodyClass([
					'cookies-set',
					this.cookiesAccepted === true ? 'cookies-accepted' : 'cookies-refused',
				]);
			...
			for (var i = 0; i < revokeButtons.length; i++) {
				revokeButtons[i].addEventListener('click', function (e) {
					...
					if (_this.noticeContainer.classList.contains('cookie-revoke-visible')) {
						...
						_this.noticeContainer.classList.contains('cookie-notice-hidden') &&
						_this.noticeContainer.classList.contains('cookie-revoke-hidden')
					)
						_this.showCookieNotice();
				});
			}

	...
	$(function () {
		...
		var btInitToken = function () {
			...
			if (paymentEl.length) {
				...
				if (typeof braintree !== 'undefined') {
					...
					// network area?
					if (cnWelcomeArgs.network === '1') ajaxArgs.cn_network = 1;
		...
		var btInitPaymentMethod = function (type) {
			if (btClient !== false) {
				if (type === 'credit_card' && btCreditCardsInitialized === false) {
					...
					btClient
						.then(btPaypalCheckoutInit)
						.then(btPaypalCheckoutSDK)
						.then(btPaypalCheckoutInstance)
						.then(btPaypalCheckoutButton)
						.catch(btGatewayFail);
				}
			} else btGatewayFail('btInitPaymentMethod btClient is false');
		};
		...
		var btHostedFieldsInstance = function (hostedFieldsInstance) {
			...
			form.on('submit', function () {
				if (form.hasClass('cn-payment-in-progress')) return false;

				form.find('.cn-form-feedback').addClass('cn-hidden');

				// spin the spinner, if exists
				if (form.find('.cn-spinner').length) form.find('.cn-spinner').addClass('spin');
				...
				if (invalidForm) {
					setTimeout(function () {
						...
						// spin the spinner, if exists
						if (form.find('.cn-spinner').length) form.find('.cn-spinner').removeClass('spin');
					}, 500);
		...
		var cnDisplayError = function (message, form) {
			if (typeof form === 'undefined') form = $('form.cn-form[data-action="payment"]');

			form
				.find('.cn-form-feedback')
				.html('<p class="cn-error">' + message + '</p>')
				.removeClass('cn-hidden');
		};
		...
		var cnWelcomeScreen = function (target) {
			...
			// network area?
			if (cnWelcomeArgs.network === '1') requestData.cn_network = 1;
		...
		var cnWelcomeForm = function (form) {
			...
			$(form[0])
				...
				.map(function (x) {
					...
					} else formData[x.name] = x.value;
				});

			// network area?
			if (cnWelcomeArgs.network === '1') formData.cn_network = 1;
		...
		$(document).on('click', '.cn-screen-button', function (e) {
			...
			// spin the spinner, if exists
			if ($(e.target).find('.cn-spinner').length) $(e.target).find('.cn-spinner').addClass('spin');

			// no form?
			if (form.length === 0) return cnWelcomeScreen(e.target);
			...
			if (formAction === 'payment') {
				...
				if (formData.plan === 'free') {
					...
					// disable form
					if (paymentEl.length) paymentEl.addClass('cn-form-disabled');

					// get subscription ID
					var subscriptionID = formData.hasOwnProperty('cn_subscription_id')
						? parseInt(formData.cn_subscription_id)
						: 0;
					...
					result.always(function (response) {
						...
						// enable form
						if (paymentEl.length) paymentEl.removeClass('cn-form-disabled');
					});
					// pro
				} else {
					// only credit cards
			...
			if (formAction === 'payment' && formData.plan === 'license') return result;
								...
								if (subscriptions.length > 0) {
									...
									for (i = 0; i < subscriptions.length; i++) {
										...
										var subscriptionText =
											subscriptions[i].VendorSubscriptionID +
											' - ' +
											licensesAvailable +
											' ' +
											cnWelcomeArgs.licensesAvailable;

										var subscriptionOption = $(
											'<option value="' + subscriptionID + '">' + subscriptionText + '</option>'
										);
		...
		$(document).on('click', '.cn-accordion > .cn-accordion-item .cn-accordion-button', function () {
			...
			var activeItem = $(this)
				.closest('.cn-accordion')
				.find('.cn-accordion-item:not(.cn-collapsed)');
		...
		$(document).on('change', 'input[name="cn_pricing_type"]', function () {
			...
			if (checked === 'yearly') $('.cn-plan-period').text(cnWelcomeArgs.paidYear);
			else $('.cn-plan-period').text(cnWelcomeArgs.paidMonth);
		...
		// init welcome modal
		if (cnWelcomeArgs.initModal == true) initModal();
	});
			...
			$(progressbar).progressbar({
				...
				create: function (event, ui) {
					timerId = setInterval(function () {
						...
						switch (currentProgress) {
							case 15:
								$(lastItem)
									.find('.cn-compliance-status')
									.addClass('cn-passed')
									.text(cnWelcomeArgs.statusPassed);
							...
							case 95:
								if (cnWelcomeArgs.complianceStatus === 'active') {
									...
									$(lastItem)
										.find('.cn-compliance-status')
										.addClass('cn-failed')
										.text(cnWelcomeArgs.statusFailed);
								}
						...
						// complete
						if (currentProgress >= 100) clearInterval(timerId);
					}, 300);
				},
				...
				complete: function (event, ui) {
					setTimeout(function () {
						if (cnWelcomeArgs.complianceStatus)
							$('.cn-compliance-check')
								.find('.cn-compliance-feedback')
								.html('<p class="cn-message">' + cnWelcomeArgs.compliancePassed + '</p>')
								.removeClass('cn-hidden');
						else
							$('.cn-compliance-check')
								.find('.cn-compliance-feedback')
								.html('<p class="cn-error">' + cnWelcomeArgs.complianceFailed + '</p>')
								.removeClass('cn-hidden');
					}, 500);
				},
			});

		...
		if (cnArgs.settingsTab === 'privacy-consent') {
			var cnListTable = {
				...
				init: function (sources) {
					for (const source in sources) {
						...
						if (sources[source].type === 'dynamic') {
							...
							this.tableContainers[source] = mainContainter.find(
								'.cn-privacy-consent-list-table-container'
							);
				...
				start: function (sourceId) {
					...
					this.tableContainers[sourceId]
						.find('.tablenav-pages a, .manage-column.sortable a, .manage-column.sorted a')
						.on('click', function (e) {
							e.preventDefault();
				...
				display: function (sourceId) {
					// already displayed?
					if (this.displayedSources.includes(sourceId)) return;
						...
						.done(function (response) {
							try {
								if (response.success) {
									...
									// update duplicated ids
									that.tableContainers[sourceId]
										.find('#the-list')
										.attr('id', 'the-list-' + sourceId);
									that.tableContainers[sourceId]
										.find('#table-paging')
										.attr('id', 'table-paging-' + sourceId);

									// bind form status handling
									that.tableContainers[sourceId]
										.find('input.cn-privacy-consent-form-status')
										.on('change', handleFormStatus);
							...
							try {
								if (response.success) {
									...
									if (response.data.column_headers.length)
										that.sourceContainers[sourceId]
											.find('thead tr, tfoot tr')
											.html(response.data.column_headers);

									if (response.data.pagination.length) {
										that.sourceContainers[sourceId]
											.find('.tablenav.bottom .tablenav-pages')
											.html($(response.data.pagination).html());
										that.sourceContainers[sourceId]
											.find('#table-paging')
											.attr('id', 'table-paging-' + sourceId);
									}
			...
			if (cnArgs.privacyConsentSources) {
				...
				for (const source in cnArgs.privacyConsentSources) {
					if (cnArgs.privacyConsentSources[source].type === 'static') {
						$('#cn_privacy_consent_' + cnArgs.privacyConsentSources[source].id)
							.find('input.cn-privacy-consent-form-status')
							.on('change', handleFormStatus);
					}
				}
				...
				$('input.cn-privacy-consent-status').on('change', function () {
					...
					if (checkbox.is(':checked')) {
						...
						checkbox
							.closest('fieldset')
							.find('.cn-privacy-consent-options-container')
							.slideDown('fast');
				...
				$('.cn-consent-log-item input[type="checkbox"]').on('change', function () {
					...
					if (el.is(':checked')) {
						...
						$(trDetailsIdHash).after(
							'<tr id="' + trDetailsRow + '" class="cn-consent-logs-row"><td colspan="6"></td></tr>'
						);
					}
				});
			...
			$('#cn_app_purge_cache a').on('click', function (e) {
				...
				$(el)
					.parent()
					.addClass('loading')
					.append('<span class="spinner is-active" style="float: none"></span>');
				...
				// network area?
				if (cnArgs.network) ajaxArgs.cn_network = 1;
			...
			$('#cn_refuse_opt').on('change', function () {
				if ($(this).is(':checked')) $('#cn_refuse_opt_container').slideDown('fast');
				else $('#cn_refuse_opt_container').slideUp('fast');
			});

			// revoke option
			$('#cn_revoke_cookies').on('change', function () {
				if ($(this).is(':checked')) $('#cn_revoke_opt_container').slideDown('fast');
				else $('#cn_revoke_opt_container').slideUp('fast');
			});

			// privacy policy option
			$('#cn_see_more').on('change', function () {
				if ($(this).is(':checked')) $('#cn_see_more_opt').slideDown('fast');
				else $('#cn_see_more_opt').slideUp('fast');
			});

			// on scroll option
			$('#cn_on_scroll').on('change', function () {
				if ($(this).is(':checked')) $('#cn_on_scroll_offset').slideDown('fast');
				else $('#cn_on_scroll_offset').slideUp('fast');
			});

			// conditional display option
			$('#cn_conditional_display_opt').on('change', function () {
				if ($(this).is(':checked')) $('#cn_conditional_display_opt_container').slideDown('fast');
				else $('#cn_conditional_display_opt_container').slideUp('fast');
			});
			...
			$(document).on('click', '.add-rule-group', function (e) {
				...
				group.append(
					'<div class="rules-group" id="rules-group-' + groupID + '">' + html + '</div>'
				);

(function ($) {
	...
	$(function () {
		...
		document.addEventListener('load.hu', function (e) {
			...
			for (const object of ['__hu', 'hu']) {
				...
				if (
					...
					window[object].hasOwnProperty('earlyInit') &&
					typeof window[object].earlyInit === 'function'
				) {
		...
		document.addEventListener('display.hu', function (e) {
			...
			$(parent.document)
				.find('input[name="cn_laws"]:checked')
				.each(function () {
					val.push($(this).val());
				});
		...
		window.addEventListener(
			...
			function (event) {
				...
				$(iframe)
					.closest('.has-loader')
					.addClass('cn-loading')
					.append('<span class="cn-spinner"></span>');
						...
						switch (option) {
							case 'position':
								$(container).removeClass(
									'hu-position-bottom hu-position-top hu-position-left hu-position-right hu-position-center'
								);
							...
							case 'laws':
								...
								if (
									$.inArray('ccpa', event.data.value) !== -1 ||
									$.inArray('otherus', event.data.value) !== -1
								) {
									var htmlElement = $(container).find('#hu-cookies-notice-dontsell-btn').parent();
							...
							case 'color_primary':
								...
								iframeContents.documentElement.style.setProperty(
									'--hu-primaryColor',
									event.data.value
								);
		...
		if (
			...
			typeof cnFrontWelcome !== 'undefined' &&
			cnFrontWelcome.previewMode
		) {
			var iframe = $(parent.document).find('#cn_iframe_id');
	...
	function cnAddPreviewModeToLink(element, iframe) {
		...
		// check special links
		if (!cnIsLinkPreviewable(element)) return;
	...
	function cnParseQueryString(string) {
		...
		_.each(string.split('&'), function (pair) {
			...
			if (_.isUndefined(parts[1])) value = null;
			else value = decodeURIComponent(parts[1].replace(/\+/g, ' '));
	...
	function cnIsLinkPreviewable(element) {
		...
		if (element.protocol !== 'https:' && element.protocol !== 'http:') return false;

		elementHost = element.host.replace(/:(80|443)$/, '');
		parsedAllowedUrl = document.createElement('a');
		matchesAllowedUrl = !_.isUndefined(
			_.find(cnFrontWelcome.allowedURLs, function (allowedUrl) {
		...
		if (!matchesAllowedUrl) return false;

		// skip wp login and signup pages
		if (/\/wp-(login|signup)\.php$/.test(element.pathname)) return false;

		// allow links to admin ajax as faux frontend URLs
		if (/\/wp-admin\/admin-ajax\.php$/.test(element.pathname)) return false;

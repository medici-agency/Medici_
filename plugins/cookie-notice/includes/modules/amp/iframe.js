...
function cnConsentResponse(event) {
	...
	if (event.type === 'set-consent.hu') document.removeEventListener('hide.hu', cnConsentResponse);
...
function cnConsentSet(categories) {
	...
	return function cnRequestFinished(event) {
		...
		if (categories[1] && !categories[2] && !categories[3] && !categories[4]) action = 'reject';

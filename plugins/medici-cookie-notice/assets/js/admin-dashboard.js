	...
	const MCNDashboard = {
		...
		initCategoryChart() {
			...
			this.charts.category = new Chart(ctx, {
				...
				data: {
					...
					datasets: [
						{
							data: [catRates.analytics || 0, catRates.marketing || 0, catRates.preferences || 0],
		...
		initGeoChart() {
			...
			this.charts.geo = new Chart(ctx, {
				type: 'doughnut',
				data: {
					labels: [i18n.eu || 'EU (GDPR)', i18n.us || 'USA', i18n.other || 'Other'],
					datasets: [
						{
							data: [geoStats.eu?.count || 0, geoStats.us?.count || 0, geoStats.other?.count || 0],
		...
		initStatusChart() {
			...
			this.charts.status = new Chart(ctx, {
				...
				data: {
					...
					datasets: [
						{
							data: [summary.accepted_all || 0, summary.rejected_all || 0, summary.customized || 0],
		...
		bindEvents() {
			...
			$('#mcn-test-banner').on('click', (e) => {
				...
				document.cookie = 'mcn_consent=; expires=Thu, 01 Jan 1970 00:00:00 UTC; path=/;';
				window.open('/', '_blank');
			});
		},

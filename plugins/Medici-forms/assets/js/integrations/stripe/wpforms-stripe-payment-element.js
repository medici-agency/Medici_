...
var WPFormsStripePaymentElement =
	window.WPFormsStripePaymentElement ||
	(function (document, window, $) {
		...
		const app = {
			...
			initializeFormsDefaultObject() {
				...
				console.warn(
					'WARNING! Function "WPFormsStripePaymentElement.initializeFormsDefaultObject()" has been deprecated, please use the "WPFormsStripePaymentElement.initializeFormDefaultObject( formId )" function instead!'
				);
			...
			setupStripeForm() {
				...
				const setupEvent = WPFormsUtils.triggerEvent(
					$(document),
					'wpformsBeforeStripePaymentElementSetup',
					[$form]
				);
			...
			handleConfirmPayment($form, result) {
				...
				if (result.paymentIntent && result.paymentIntent.status === 'succeeded') {
					...
					$form.append(
						'<input type="hidden" class="wpforms-stripe-payment-intent-id" name="wpforms[payment_intent_id]" value="' +
							result.paymentIntent.id +
							'">'
					);
					$form.append(
						'<input type="hidden" class="wpforms-stripe-payment-link-email" name="wpforms[payment_link_email]" value="' +
							app.forms[formId].linkEmail +
							'">'
					);
			...
			setupPaymentElement($form) {
				...
				WPFormsUtils.triggerEvent($(document), 'wpformsStripePaymentElementInitialized', [
					$form,
					app.forms,
				]);
			},
			 ...
			 */
			processConditionalsField(formID, fieldID, pass, action) {
			 ...
			 */
			getElementAppearanceOptions($form) {
				...
				const inputStyle = {
					borderColor:
						app.getCssPropertyValue($hiddenInput, '--field-border') ||
						app.getCssPropertyValue($hiddenInput, 'border-color'),
					borderRadius: app.getCssPropertyValue($hiddenInput, 'border-radius'),
					fontSize: app.getCssPropertyValue($hiddenInput, 'font-size'),
					colorText:
						app.getCssPropertyValue($hiddenInput, '--secondary-color') ||
						app.getCssPropertyValue($hiddenInput, 'color'),
					colorTextPlaceholder:
						app.getCssPropertyValue($hiddenInput, '--secondary-color-50') ||
						WPFormsUtils.cssColorsUtils.getColorWithOpacity(
							app.getCssPropertyValue($hiddenInput, 'color'),
							'0.5'
						),
					colorBackground:
						app.getCssPropertyValue($hiddenInput, '--background-color') ||
						app.getCssPropertyValue($hiddenInput, 'background-color'),
					fontFamily: app.getCssPropertyValue($hiddenInput, 'font-family'),
					errorColor: '#990000',
				};

				inputStyle.colorBackground = WPFormsUtils.cssColorsUtils.rgbaToHex(
					inputStyle.colorBackground
				);
				inputStyle.borderColor = WPFormsUtils.cssColorsUtils.isValidColor(inputStyle.borderColor)
			...
			initializePaymentElement($form, email = '') {
				...
				app.forms[formId].paymentElement = app.forms[formId].elements.create('payment', {
			...
			getMappedLinkEmailField($form) {
				const linkEmailMappedFieldId = $form
					.find('.wpforms-field-stripe-credit-card .wpforms-field-row')
					.data('link-email');
			...
			submitHandler(form) {
				...
				const valid = $form.validate().form(),
					...
					cardFilled =
						(!$stripeRow.data('link-email') && app.forms[formId].elementsModified) ||
						$stripeRow.data('completed') ||
						mobilePayment;
			...
			updateElementsTotalAmount(e, $form, total) {
				...
				app.forms[formId].elements.update({
					amount: parseInt(wpforms.numberFormat(total, currency.decimals, '', ''), 10),
				});
			},
					...
					.then(function (result) {
						...
						$form.append(
							'<input type="hidden" class="wpforms-stripe-payment-method-id" name="wpforms[payment_method_id]" value="' +
								app.forms[formId].paymentMethodId +
								'">'
						);
			 ...
			 */
			pageChange(event, currentPage, $form, action) {
				// eslint-disable-line complexity
				...
				if (
					...
					(app.forms[formId].lockedPageToSwitch &&
				...
				const linkCompleted =
					typeof $stripeDiv.data('linkCompleted') !== 'undefined'
						? $stripeDiv.data('linkCompleted')
						: true;
			...
			getCssVar(style, varName) {
				// eslint-disable-next-line no-console
				console.warn(
					'WARNING! Function "WPFormsStripePaymentElement.getCssVar()" has been deprecated, please use the "WPForms.FrontendModern.getCssVar()" function instead!'
				);
			...
			updatePaymentElementStylesModern($form) {
				...
				if (
					!$.isEmptyObject(app.getCustomAppearanceOptions()) ||
					!window.WPForms ||
			 ...
			 */
			updateFormElementsAppearance(formElements, cssVars) {
				...
				cssVars['field-size-padding-v'] =
					(parseInt(cssVars['field-size-input-height'], 10) -
						parseInt(cssVars['field-size-font-size'], 10) -
						6) /
						2 +
					'px';
				...
				appearance.variables.colorText = WPForms.FrontendModern.getSolidColor(
					cssVars['field-text-color']
				);

				let maybeMenuBgColor = !WPFormsUtils.cssColorsUtils.isTransparentColor(
					cssVars['field-background-color']
				)
					? cssVars['field-background-color']
					: cssVars['field-menu-color'];
				...
				appearance.rules = {
					'.Input': {
						border:
							cssVars['field-border-size'] +
							' ' +
							cssVars['field-border-style'] +
							' ' +
							cssVars['field-border-color'],
					...
					'.Tab': {
						border:
							'1px solid ' +
							WPForms.FrontendModern.getColorWithOpacity(cssVars['field-border-color'], '0.5'),
						borderRadius: cssVars['field-border-radius'],
						backgroundColor: 'transparent',
						boxShadow: 'none',
						marginTop: '0',
					...
					'.Tab:focus': {
						border:
							'1px solid ' +
							WPForms.FrontendModern.getColorWithOpacity(cssVars['button-background-color'], '0.5'),
			...
			getCustomAppearanceVariables($form) {
				...
				app.forms[formId].customVars = {
					focusColor:
						app.getCssPropertyValue($hiddenInput, '--accent-color') ||
						app.getCssPropertyValue($hiddenInput, 'color'),
					borderColorWithOpacity: WPFormsUtils.cssColorsUtils.getColorWithOpacity(
						primaryColor,
						'0.1'
					),
				};
			...
			getElementPrimaryColor($cardInput) {
				const primaryColor =
					app.getCssPropertyValue($cardInput, '--primary-color') ||
					app.getCssPropertyValue($cardInput, 'color');

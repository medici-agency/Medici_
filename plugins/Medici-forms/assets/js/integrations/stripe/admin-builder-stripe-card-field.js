	...
	(function (document, window, $) {
		...
		const app = {
			...
			bindUIActions: function () {
				...
				$(document).on(
					'change',
					'.wpforms-field-option-stripe-credit-card .wpforms-field-option-row-sublabel_position select',
					app.sublabelPositionChange
				);
				$(document).on(
					'change',
					'.wpforms-field-option-stripe-credit-card .wpforms-field-option-row-link_email select',
					app.linkEmailChange
				);
			...
			ajaxRequiredCheck: function () {
				if (
					!$('.wpforms-field.wpforms-field-' + wpforms_builder_stripe.field_slug).length ||
			...
			paymentsEnabledCheck: function () {
				if (
					!$(`.wpforms-field.wpforms-field-${wpforms_builder_stripe.field_slug}:visible`).length ||
			...
			disableAddCardButton: function (e, id, type) {
				if (wpforms_builder_stripe.field_slug === type) {
					$('#wpforms-add-fields-' + wpforms_builder_stripe.field_slug).prop('disabled', true);
			...
			enableAddCardButton: function (e, id, type) {
				if (wpforms_builder_stripe.field_slug === type) {
					$('#wpforms-add-fields-' + wpforms_builder_stripe.field_slug).prop('disabled', false);
				}
			},
			...
			linkEmailChange: function () {
				...
				$(`#wpforms-field-${fieldId}`)
					.find('.wpforms-stripe-link-email')
					.toggleClass('wpforms-hidden', $(this).val() !== '');
			},

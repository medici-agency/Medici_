 ...
 * @since 1.9.5
 */
const WPFormsBuilderSquare =
	window.WPFormsBuilderSquare ||
	(function (document, window, $) {
		...
		const app = {
			...
			bindUIActions() {
				$(document)
					.on('wpformsSaved', app.ajaxRequiredCheck)
			...
			bindPlanUIActions() {
				el.$panelContent
					.find(
						'.wpforms-panel-content-section-payment-plan-body .wpforms-panel-field-select select'
					)
					.on('change', app.resetRequiredPlanFieldError);
			},
			...
			requiredFieldsCheck() {
				if (
					!el.$recurringPaymentControl.is(':checked') ||
					el.$panelContent.hasClass('wpforms-hidden')
				) {
					return;
				}
			...
			settingsRedirect() {
				...
				window.location.href =
					...
					'#wpforms-panel-field-square-enable_recurring-wrap';
			...
			toggleMultiplePlansWarning() {
				el.$panelContent
					.find('.wpforms-square-multiple-plans-warning')
					.toggleClass(
						'wpforms-hidden',
			...
			disablePayments() {
				const toggleInput = $(
					'#wpforms-panel-field-square-enable_one_time, #wpforms-panel-field-square-enable_recurring'
				);
			...
			disableNotifications() {
				const $notificationWrap = $(
					'.wpforms-panel-content-section-notifications [id*="-square-wrap"]'
				);
			...
			isPaymentsEnabled() {
				return (
					el.$singlePaymentControl.is(':checked') || el.$recurringPaymentControl.is(':checked')
				);
			},

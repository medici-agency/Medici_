...
WPFormsChallenge.builder =
	window.WPFormsChallenge.builder ||
		...
		var app = {
			...
			events: function () {
				...
				$('#wpforms-builder')
					...
					.on(
						'wpformsPanelSwitch wpformsPanelSectionSwitch wpformsBuilderPanelLoaded',
						function () {
							WPFormsChallenge.core.updateTooltipUI();
						}
					);
				...
				$(document).on(
					...
					function (event, originalClickEvent) {
				...
				$(document).on('wpformsBuilderReady', function () {
					if (
						$('.wpforms-panel-fields-button').hasClass('active') &&
						WPFormsChallenge.core.loadStep() <= 2
					) {
						WPFormsChallenge.core.stepCompleted(1);
			...
			gotoStep: function (step) {
				console.warn(
					'WARNING! Function "WPFormsChallenge.builder.gotoStep()" has been deprecated.'
				);
			},
			...
			gotoNotificationStep: function (e) {
				WPFormsChallenge.core.stepCompleted(3).done(function () {
					...
					WPFormsBuilder.panelSectionSwitch(
						$('.wpforms-panel .wpforms-panel-sidebar-section-notifications')
					);
				});
			},
			...
			showEmbedPopup: function () {
				WPFormsChallenge.core.stepCompleted(4).done(WPFormsFormEmbedWizard.openPopup);
			},

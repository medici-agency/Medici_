			...
			cancelChallenge: function () {
				...
				if (typeof WPFormsBuilder !== 'undefined') {
					WPFormsChallenge.admin
						...
						.done(function () {
							// Save the form before removing scripts if we're in a WPForms Builder.
							if (localStorage.getItem('wpformsChallengeStep') !== null) {
								WPFormsBuilder.formSave(false);
							}
						})
						.done(
							// Remove scripts related to challenge.
							$(
								'#wpforms-challenge-admin-js, #wpforms-challenge-core-js, #wpforms-challenge-admin-js-extra, #wpforms-challenge-builder-js'
							).remove()
						);
				} else {
					WPFormsChallenge.admin.saveChallengeOption(optionData).done(app.triggerPageSave); // Assume we're on form embed page.
				}
			},

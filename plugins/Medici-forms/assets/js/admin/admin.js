...
(function ($) {
	...
	var WPFormsAdmin = {
		...
		ready() {
			...
			$('#screen-options-wrap .hide-column-tog').on(
				'change',
				WPFormsAdmin.handleOnChangeScreenOptions
			);
		},
		...
		initChoicesJS() {
			$('.choicesjs-select').each(function () {
				const $this = $(this),
					args = window.wpforms_admin_choicesjs_config
						? { ...window.wpforms_admin_choicesjs_config }
						: {};

				if ($this.attr('multiple')) {
					args.removeItemButton =
						typeof args.removeItemButton !== 'undefined' ? args.removeItemButton : true;
				}
		...
		initCheckboxMultiselectColumns() {
			$(document).on('change', '.checkbox-multiselect-columns input', function () {
				...
				if ($this.prop('checked')) {
					...
					if (!$item.length) {
						$container
							.find('.second-column ul')
							.append('<li id="' + itemID + '">' + label + '</li>');
					}
				} else {
			...
			$(document).on('click', '.checkbox-multiselect-columns .all', function (event) {
				...
				$(this)
					.closest('.checkbox-multiselect-columns')
		...
		initFormOverview() {
			...
			console.warn(
				'WARNING! Function "WPFormsAdmin.initFormOverview()" has been deprecated, please use the new "WPFormsForms.Overview.init()" function instead!'
			);
		...
		initEntriesList() {
			...
			$(document).on('click', '#wpforms-entries-table #doaction', function (event) {
				...
				const $content =
					'delete' === $action.val()
						? wpforms_admin.entry_delete_n_confirm
						: wpforms_admin.entry_trash_n_confirm;
			...
			$(document).on(
				...
				function (event) {
					...
					if (
						!$table.hasClass('wpforms-entries-table-spam') &&
						!$table.hasClass('wpforms-entries-table-trash')
					) {
						$counter.text(total);
					}
			...
			$(document).on(
				'click',
				'#wpforms-entries-list .form-details-actions-removeall',
				function (event) {
					event.preventDefault();
					...
					const $page = $(this).data('page'),
						...
						filteredCount =
							$table.data('filtered-count-trash') && $noticeData.action === 'trash'
								? parseInt($table.data('filtered-count-trash'), 10)
								: 0,
						data = {
					...
					$.confirm({
						title: wpforms_admin.heads_up,
						content:
							filteredCount && $('#wpforms-reset-filter').length
								? $noticeData.content.replace('{entry_count}', filteredCount)
								: $noticeData.contentAll,
						icon: 'fa fa-exclamation-circle',
						type: 'orange',
						buttons: {
							confirm: {
								...
								keys: ['enter'],
								action: () => {
									$.get(wpforms_admin.ajax_url, data).done(function (response) {
										if (response.success) {
			...
			$(document).on('heartbeat-tick', function (event, data) {
				...
				$link.text(data.wpforms_new_entries_notification).slideDown({
					start() {
		...
		initEntriesSingle() {
			...
			if (
				'wpforms-entries' === WPFormsAdmin.getQueryString('page') &&
				'details' === WPFormsAdmin.getQueryString('view')
			) {
			...
			$(document).on(
				'click',
				'#wpforms-entries-single .wpforms-empty-field-toggle',
				function (event) {
					event.preventDefault();
			...
			$(document).on(
				'click',
				'#wpforms-entries-single .wpforms-entry-notes-new .add',
				function (event) {
					event.preventDefault();
			...
			$(document).on(
				'click',
				'#wpforms-entries-single .wpforms-entry-notes-new .cancel',
				function (event) {
					event.preventDefault();
			...
			$(document).on(
				'click',
				'#wpforms-entries-single .wpforms-entry-notes-byline .note-delete',
				function (event) {
					event.preventDefault();
		...
		entryHotkeys() {
			$(document).on('keydown', function (event) {
				if (
					74 === event.keyCode &&
					!event.metaKey &&
					!WPFormsAdmin.isFormTypeNode(event.target.nodeName)
				) {
		...
		initWelcome() {
			...
			$(document).on('click', '#wpforms-welcome .play-video', function (event) {
				...
				const video =
					'<div class="video-container"><iframe width="1280" height="720" src="https://www.youtube-nocookie.com/embed/SQ9kV9SKz5k?rel=0&amp;showinfo=0&amp;autoplay=1" frameborder="0" allowfullscreen></iframe></div>';
		...
		initAddons() {
			...
			if ($sectionAll.length || $sectionInstalled.length) {
				...
				if ($sectionInstalled.length) {
					addonSearchInstalled = new List('wpforms-addons-list-section-installed', {
						valueNames: ['addon-link'],
					});
				}

				if ($sectionAll.length) {
					addonSearchAll = new List('wpforms-addons-list-section-all', {
						valueNames: ['addon-link'],
					});
				}

				$('#wpforms-addons-search').on('keyup search', function () {
			...
			$(document).on(
				'change',
				'.wpforms-addons-list-item .wpforms-toggle-control input',
				function (event) {
					event.preventDefault();
		...
		updateAddonSearchResult(searchField, addonSearchAll, addonSearchInstalled) {
			...
			const searchResultsInstalled = addonSearchInstalled
				? addonSearchInstalled.search(searchTerm)
				: [];

			$noResultsMessage.toggle(
				searchResultsAll.length === 0 && searchResultsInstalled.length === 0
			);
		...
		addonToggleNew($btn) {
			...
			function handleError(res) {
				...
				if (typeof res.data === 'object') {
					$footer.append(
						`<div class="wpforms-addons-list-item-footer-error"><p>${pluginType === 'addon' ? wpforms_admin.addon_error : wpforms_admin.plugin_error}</p></div>`
					);
				} else {
					$footer.append(
						`<div class="wpforms-addons-list-item-footer-error"><p>${res.data}</p></div>`
					);
				}
		...
		addSpinnerToButton($button) {
			const spinnerBlue =
				'<i class="wpforms-loading-spinner wpforms-loading-blue wpforms-loading-inline"></i>';
		...
		updateAddonButtonPropertiesAndUI($btn, $addon, $footer, classes, checked) {
			...
			$btn
				.siblings('.wpforms-toggle-control-status')
				.html($btn.siblings('.wpforms-toggle-control-status').data(checked ? 'on' : 'off'));
					...
					if (res.success) {
						if ('install' === state) {
							...
							if (!res.data.is_activated) {
								...
								buttonText =
									'plugin' === pluginType
										? wpforms_admin.addon_activate
										: s.iconActivate + wpforms_admin.addon_activate;
								cssClass =
									'plugin' === pluginType
						...
						if ('object' === typeof res.data) {
							if (pluginType === 'addon') {
								$addon
									.find('.actions')
									.append('<div class="msg error"><p>' + wpforms_admin.addon_error + '</p></div>');
							} else {
								$addon
									.find('.actions')
									.append('<div class="msg error"><p>' + wpforms_admin.plugin_error + '</p></div>');
							}
						} else {
							$addon
								.find('.actions')
								.append('<div class="msg error"><p>' + res.data + '</p></div>');
						}
		...
		initSettings() {
			...
			$(document).on('wpformsReady', function () {
				...
				if (integrationFocus) {
					...
					$('body').animate({ scrollTop: $('#' + jumpTo).offset().top }, 1000);
				}
				...
				$('.wpforms-admin-settings-form').conditions([
					// Misc > Disable User Cookies visibility.
					{
						...
						actions: {
							if: {
								element:
									'#wpforms-setting-row-gdpr-disable-uuid,#wpforms-setting-row-gdpr-disable-details',
								action: 'show',
							},
							else: {
								element:
					...
					// CAPTCHA > Type.
					{
						...
						actions: {
							if: [
								...
								{
									element:
										'.wpforms-setting-recaptcha, #wpforms-setting-row-recaptcha-site-key, #wpforms-setting-row-recaptcha-secret-key, #wpforms-setting-row-recaptcha-fail-msg, .wpforms-setting-turnstile, #wpforms-setting-row-turnstile-heading, #wpforms-setting-row-turnstile-site-key, #wpforms-setting-row-turnstile-secret-key, #wpforms-setting-row-turnstile-theme, #wpforms-setting-row-turnstile-fail-msg',
									action: 'hide',
								},
							],
						},
					...
					},
					{
						...
						actions: {
							if: [
								...
								{
									element:
										'#wpforms-setting-row-hcaptcha-heading, #wpforms-setting-row-hcaptcha-site-key, #wpforms-setting-row-hcaptcha-secret-key, #wpforms-setting-row-hcaptcha-fail-msg, #wpforms-setting-row-turnstile-heading, #wpforms-setting-row-turnstile-site-key, #wpforms-setting-row-turnstile-secret-key, #wpforms-setting-row-turnstile-theme, #wpforms-setting-row-turnstile-fail-msg',
									action: 'hide',
								},
							],
						},
					...
					},
					{
						...
						actions: {
							if: [
								...
								{
									element:
										'#wpforms-setting-row-hcaptcha-heading, #wpforms-setting-row-hcaptcha-site-key, #wpforms-setting-row-hcaptcha-secret-key, #wpforms-setting-row-hcaptcha-fail-msg, .wpforms-setting-recaptcha, #wpforms-setting-row-recaptcha-site-key, #wpforms-setting-row-recaptcha-secret-key, #wpforms-setting-row-recaptcha-fail-msg',
									action: 'hide',
								},
							],
						},
					...
					},
					{
						...
						actions: {
							if: [
								...
								{
									element:
										'.wpforms-setting-captcha-heading, #wpforms-setting-row-captcha-provider',
									action: 'show',
								},
							],
						},
			...
			$(document).on(
				'change',
				'#wpforms-setting-row-render-engine input',
				WPFormsAdmin.settingsRenderEngineChange
			);
			...
			$(document).on(
				'click',
				'.wpforms-settings-provider-accounts-list .remove a',
				function (event) {
					event.preventDefault();
			...
			$(document).on(
				'click',
				'.wpforms-settings-provider:not(.focus-out) .wpforms-settings-provider-header',
				function (event) {
					event.preventDefault();
					...
					$this
						...
						.slideToggle('', function () {
							$this
								.parent()
								.find('.wpforms-settings-provider-logo i')
			...
			$(document).on('change', '#wpforms-setting-row-recaptcha-type input', function () {
				$('#wpforms-setting-row-captcha-preview').toggleClass(
					'wpforms-hidden',
					'v2' !== this.value
				);
				$('#wpforms-setting-row-recaptcha-v3-threshold').toggleClass(
					'wpforms-hidden',
					'v3' !== this.value
				);
			});
		...
		licenseDeactivate(el) {
			...
			$.post(wpforms_admin.ajax_url, data, function (res) {
				...
				const msg =
					!resData.msg || typeof resData.msg !== 'string'
						? wpforms_admin.something_went_wrong
						: resData.msg;

				if (res.success) {
					$row
						.find('#wpforms-setting-license-key')
						.val('')
						.attr('value', '')
		...
		licenseRefresh(el) {
			...
			$.post(wpforms_admin.ajax_url, data, function (res) {
				...
				if (res.success) {
					...
					$input
						.removeClass('wpforms-setting-license-is-valid')
						.addClass('wpforms-setting-license-is-invalid');
				}
						...
						action() {
							$.post(wpforms_admin.ajax_url, data, function (response) {
								if (response.success) {
									...
									const numberOfIntegrations = $provider.find(
										'.wpforms-settings-provider-accounts-list li'
									).length;
		...
		initTools() {
			...
			$(document).on(
				'change',
				'#wpforms-tools-form-import, #wpforms-tools-form-other-import, #wpforms-tools-form-export, #wpforms-tools-form-template',
				function () {
					const $field = $(this);
		...
		verifySSLConnection() {
			...
			$.post(wpforms_admin.ajax_url, data, function (res) {
				...
				if (res.success) {
					$btn.before(
						'<div class="notice wpforms-notice notice-success">' + res.data.msg + '</div>'
					);
				}
			...
			$(window).on(
				...
				_.debounce(function () {
					const wpfooterTop = $wpfooter.offset().top,
						...
						overlapBottom =
							$overlap.length > 0 ? $overlap.offset().top + $overlap.height() + 85 : 0,

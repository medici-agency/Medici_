	...
	(function (document, window, $) {
		...
		var app = {
			...
			events: function () {
				...
				el.$categories.on(
					'click',
					'.wpforms-builder-help-category button.viewall',
					event.viewAllCategoryDocs
				);
			...
			getBuilderContext: function () {
				...
				switch (panel) {
					case 'fields':
						section = $panel
							.find('.wpforms-panel-sidebar .wpforms-tab a.active')
							.parent()
							.attr('id');
						break;
					case 'setup':
				...
				if (section === 'field_options') {
					subsection = $panel
						.find(
							'#wpforms-field-options .wpforms-field-option:visible .wpforms-field-option-hidden-type'
						)
						.val();
				}
		...
		event = {
			...
			viewAllCategoryDocs: function (e) {
				...
				$btn
					.prev('div')
					.stop()
					.slideToggle(ui.config.speed, function () {

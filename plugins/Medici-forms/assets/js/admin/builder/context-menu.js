	...
	(function (document, window, $) {
		...
		const app = {
			...
			selectors: {
				...
				contextMenuItem:
					'.wpforms-context-menu:not(.wpforms-context-menu-dropdown) .wpforms-context-menu-list-item',
			...
			events() {
				...
				el.$mainContextMenu.on(
					'click',
					'.wpforms-context-menu-list-item',
					app.mainMenuItemClickAction
				);
			...
			rightClickContextMenuHandler(e) {
				...
				if (
					!$panel.length ||
					!$field.length ||
					$(e.target).closest(app.selectors.contextMenu).length
				) {
					return;
				}
			...
			hideMainContextMenu(event) {
				if (
					el.$mainContextMenu.is(':hidden') ||
					$(event.target).closest(app.selectors.mainContextMenuContainer).length > 0
				) {
					return;
				}
			...
			handleEditAction($field, fieldId) {
				...
				$(`#wpforms-field-option-basic-${fieldId} .wpforms-field-option-group-toggle`).trigger(
					'click'
				);
			},
			...
			handleSmartLogicAction($field, fieldId) {
				...
				$field.trigger('click');
				$(
					`#wpforms-field-option-conditionals-${fieldId} .wpforms-field-option-group-toggle`
				).trigger('click');
				$(
					`#wpforms-field-option-${fieldId} .wpforms-field-option-group-conditionals .education-modal`
				).trigger('click');
			},
			...
			handleSizeAction($item, fieldId) {
				...
				$item
					.addClass('wpforms-context-menu-list-item-active')
					.siblings()
					.removeClass('wpforms-context-menu-list-item-active');
			},
			...
			shouldHideLabel(fieldId) {
				...
				return (
					$label.length === 0 ||
					$label.parents('.wpforms-field-option-row').hasClass('wpforms-disabled')
				);
			},
			...
			shouldHideFieldSize(fieldId, $field) {
				...
				return (
					...
					isFieldInColumn ||
					isRepeaterField ||
					$size.parent().hasClass('wpforms-hidden')
				);
			},
			...
			shouldHideSmartLogic(fieldId) {
				return (
					$(`#wpforms-field-option-conditionals-${fieldId}`).length === 0 &&
					$(
						`#wpforms-field-option-${fieldId} .wpforms-field-option-group-conditionals .education-modal`
					).length === 0
				);
			...
			checkDividerVisibility() {
				el.$contextMenuDivider.each(function () {
					...
					visibility.split(',').forEach(function (item) {
						if (
							$('.wpforms-context-menu-list-item[data-action="' + item.trim() + '"]').css(
								'display'
							) !== 'none'
						) {
							shouldHide = false;

...
WPForms.Admin.Builder.FieldNumbers =
	WPForms.Admin.Builder.FieldNumbers ||
	(function (document, window, $) {
		...
		const app = {
			...
			numbersEvents() {
				...
				app.$builder.on('click', '.wpforms-smart-tags-widget .tag', app.smartTagClickTracking);
			},
			...
			adjustValue($sourceField, $targetField) {
				$targetField
					.val(app.parseFieldValue($sourceField))
					.trigger('input')
					.trigger('wpformsSmartTagsInputSync');
			},
			...
			onChangeNumbersMin(event) {
				const $minField = $(event.target),
					...
					$defaultValueField = $container.find(
						'.wpforms-field-option-row-default_value input.wpforms-smart-tags-widget-original'
					);

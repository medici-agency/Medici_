...
WPForms.Admin.Builder.SmartTags =
	WPForms.Admin.Builder.SmartTags ||
	(function (document, window, $) {
		...
		const app = {
			...
			events() {
				...
				el.$builder.on(
					'click',
					'.wpforms-show-smart-tags, .mce-wpforms-smart-tags-mce-button',
					function () {
			...
			initWidgets($scope = el.$builder) {
				...
				$smartTagsInputs.each(function () {
					...
					// Create a new div with the required attributes and content.
					const $widget = $('<div>', {
						class: ['wpforms-smart-tags-widget', widgetTypeClass, readonlyClass]
							.filter(Boolean)
							.join(' '),
					...
					$widgetContainer.append(
						'<span class="wpforms-show-smart-tags"><i class="fa fa-tags"></i></span>'
					);
			...
			reinitWidgets($scope) {
				...
				$scope.find('.wpforms-smart-tags-widget-container').each(function () {
					const $this = $(this);
					$this
						.next('.wpforms-smart-tags-enabled')
						.removeClass('wpforms-smart-tags-widget-original');
					$this.remove();
				});
			 ...
			 */
			renderWidgetContent(e, forceUpdate = false, cursorPosition = '') {
			...
			getSmartTagTitle(value) {
				...
				const tagTitle =
					app.getSmartTagFieldTitle(value) ||
			...
			getSmartTagFieldTitle(value) {
				...
				if (fieldId) {
					const subFieldCapitalize = subField.length
				...
				return wpforms_builder.smart_tags_templates[tag].replace('%1$s', fieldIdLabelString);
			},
			...
			getSmartTagWithArgsTitle(value) {
				...
				const matches = value.match(
					/^(query_var|user_meta|date|entry_date) (key|format)="([^"]+)?"$/
				);
				...
				// Return formatted Smart Tag title.
				return wpforms_builder.smart_tags_templates[tag].replace('%1$s', key);
			},
			...
			updateSmartTagsTitles($scope) {
				$scope.find('.wpforms-smart-tags-widget .tag').each(function () {
					...
					$tag.text(app.getSmartTagTitle($tag.data('value'))).append($close);
				});
			},
			...
			smartTagClick(e) {
				...
				// language=HTML
				const $editOk = $('<i class="tag-edit-ok fa fa-check-circle"></i>').attr(
					'title',
					wpforms_builder.smart_tags_edit_ok_button
				);
			...
			smartTagBlur($tag, restore = false) {
				...
				$tag
					...
					.attr('contenteditable', false)
					.next('.tag-edit-ok')
					.remove(); // Remove the Ok button (check icon).

				const $widget = $tag.parent();

				$widget.attr('contenteditable', true).trigger('input');
			...
			initDropdowns() {
				el.$builder
					.find('.wpforms-show-smart-tags, .mce-wpforms-smart-tags-mce-button')
					.each(function () {
						const $button = $(this),
			...
			getDropdownListInstance($button) {
				...
				const originalInput = isMceButton
					? $button.closest('.wp-editor-container').find('textarea')
					: $button
							.closest('.wpforms-smart-tags-widget-container')
							.next('.wpforms-smart-tags-widget-original');
			...
			insertSmartTagToTinyMCE(dropdownInstance, smartTag) {
				...
				const inputId = dropdownInstance.$button
					.closest('.wp-editor-container')
					.find('textarea')
					.attr('id');
			...
			scrollToCursorPosition(container) {
				...
				setTimeout(() => {
					...
					const isMarkerHorizontallyVisible =
						markerRect.left >= containerRect.left && markerRect.right <= containerRect.right;

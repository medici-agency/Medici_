...
WPForms.Admin.Builder.WPFormsChoicesJS =
	WPForms.Admin.Builder.WPFormsChoicesJS ||
		...
		const app = {
			...
			setupSearchEvents($element, choicesJS, ajaxArgs) {
				...
				searchInput?.addEventListener('keyup', function (ev) {
					...
					if ((ev.which !== 8 && ev.which !== 46) || ev.target.value.length > 0) {
						return;
					}
				...
				choicesJS.passedElement?.element.addEventListener(
					'search',
			...
			performSearch(choicesJS, searchTerm, ajaxArgs) {
				...
				const requestSearchChoices = app.ajaxSearch(
					ajaxArgs.action,
					searchTerm,
					ajaxArgs.nonce,
					choicesJS.getValue(true)
				);
			...
			ajaxSearchPages(action, searchTerm, nonce) {
				...
				console.warn(
					'WPForms.Admin.Builder.WPFormsChoicesJS.ajaxSearchPages is deprecated. Use WPForms.Admin.Builder.WPFormsChoicesJS.ajaxSearch instead.'
				);
			...
			ajaxSearch(action, searchTerm, nonce, exclude = []) {
				...
				return $.get(wpforms_builder.ajax_url, args).fail(function (err) {

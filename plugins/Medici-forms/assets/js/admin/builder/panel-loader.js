...
WPForms.Admin.Builder.PanelLoader =
	WPForms.Admin.Builder.PanelLoader ||
				...
				$.post(wpforms_builder.ajax_url, {
					...
					.done(function (response) {
						if (!response.success || !response.data?.length) {
							...
							app.displayErrorModal(
								`<p>${wpforms_builder.error_load_templates}</p><p>${wpforms_builder.error_contact_support}</p>`
							);
					...
					.fail(function () {
						// Show an error message.
						app.displayErrorModal(
							`<p>${wpforms_builder.something_went_wrong}.</p><p>${wpforms_builder.error_contact_support}</p>`
						);
					})
					.always(function () {

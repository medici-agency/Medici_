...
var WPFormsEditPostEducation =
	window.WPFormsEditPostEducation ||
	(function (document, window, $) {
		...
		const app = {
			...
			getPostTitle() {
				...
				const { title = '' } =
					select('core').getEditedEntityRecord('postType', context.postType, context.postId) || {};
			...
			getGutenbergNoticeSettings() {
				...
				noticeSettings.actions[0].onClick = () =>
					registerPlugin(app.pluginId, { render: GutenbergTutorial });
			...
			getGuidePages() {
				...
				wpforms_edit_post_education.gutenberg_guide.forEach(function (page) {
					pages.push({
						...
						image: (
							<img className="edit-post-welcome-guide__image" src={page.image} alt={page.title} />
						),

...
WPForms.FrontendModern =
	...
	(function (document, window, $) {
		...
		const app = {
			 ...
			 */
			events() {
				$(document).on('wpforms_elementor_form_fields_initialized', app.initPageBreakButtons);
				...
				$('form.wpforms-form .wpforms-submit').on('keydown click', app.disabledButtonPress);

				// Add styling to timepicker dropdown.
				$(document).on(
					'focus',
					'.wpforms-render-modern .wpforms-timepicker',
					app.updateTimepickerDropdown
				);
			...
			formSubmitButtonDisable(e, $form, $submitBtn) {
				...
				$submitBtn.before(
					`<div class="wpforms-hidden" id="${disabledBtnDescId}">${wpforms_settings.submitBtnDisabled}</div>`
				);
			...
			pageChange(e, nextPage, $form) {
				...
				if ($pageIndicator.hasClass('progress')) {
					pageTitle = $pageIndicator
						.find('.wpforms-page-indicator-page-title')
						.data(`page-${nextPage}-title`);
				} else {
					pageTitle = $pageIndicator
						.find(`.wpforms-page-indicator-page-${nextPage} .wpforms-page-indicator-page-title`)
						.text();
				}
			...
			getCssVars($form) {
				...
				const $cont = $form.hasClass('wpforms-container')
					? $form

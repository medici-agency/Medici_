...
WPFormsEducation.liteCore =
	window.WPFormsEducation.liteCore ||
		...
		const app = {
			...
			openModalButtonClick() {
				$(document)
					...
					.on(
						'mousedown',
						'.education-modal.wpforms-add-fields-button',
						app.openModalButtonHandler
					);
			},
			...
			upgradeModal(feature, utmContent, type, video, isPlural) {
				...
				const titleMessage = isPlural
					? wpforms_education.upgrade[type].title_plural
					: wpforms_education.upgrade[type].title;
				...
				const modal = $.alert({
					...
					onOpenBefore() {
						...
						const videoHtml = isVideoModal
							? '<iframe src="' +
								video +
								'" class="feature-video" allowfullscreen="" width="475" height="267"></iframe>'
							: '';

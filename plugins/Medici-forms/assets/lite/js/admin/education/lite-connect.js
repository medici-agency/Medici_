...
WPFormsEducation.liteConnect =
	window.WPFormsEducation.liteConnect ||
	(function (document, window, $) {
		...
		const app = {
			...
			initLiteConnectToggle() {
				$('.wpforms-toggle-control.wpforms-setting-lite-connect-auto-save-toggle input').prop(
					'disabled',
					false
				);
			},
				...
				$(document).on(
					...
					function (event) {
						...
						const wrapper = $(this).closest(
							'#wpforms-setting-row-lite-connect-enabled, .wpforms-setting-lite-connect-auto-save-toggle'
						);
			...
			enableLiteConnectButtonClick() {
				$(document).on('click', '.wpforms-dyk-lite-connect .button-primary', function (event) {
					event.preventDefault();
					...
					app.openSettingsLiteConnectModal(false, app.enableLiteConnectButtonModalConfirm);
				});
			},
			...
			enableLiteConnectAIButtonClick() {
				$(document).on('click', '.enable-lite-connect-modal', app.handleLiteConnectModalClick);
			},
			...
			openAILiteConnectEnableModal(confirmCallback) {
				...
				wpforms_education_lite_connect.update_result.enabled_title =
					wpforms_education_lite_connect.enable_ai.enabled_title;
							...
							action() {
								...
								if (window.WPFormsChallenge) {
									...
									var completeChallenge =
										WPFormsChallenge.embed && WPFormsChallenge.embed.completeChallenge;
								}
			...
			saveSettingAjaxPost(isEnabled, $toggle, successCallback) {
				...
				$.post(wpforms_education_lite_connect.ajax_url, {
			...
			toggleBuilderTopBar(open) {
				...
				root.setAttribute('style', cssVar + ': ' + adminBarHeight + 'px!important;');
			},
